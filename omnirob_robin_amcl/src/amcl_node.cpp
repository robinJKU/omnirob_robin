...
#include "omnirob_robin_amcl/omnirob_robin_AMCLConfig.h"
...
class AmclNode
{
  ...
  private:
    ...
    dynamic_reconfigure::Server<omnirob_robin_amcl::omnirob_robin_AMCLConfig> *dsrv_;
    omnirob_robin_amcl::omnirob_robin_AMCLConfig default_config_;
    ros::Timer check_laser_timer_;

    int max_beams_, min_particles_, max_particles_;
    double alpha1_, alpha2_, alpha3_, alpha4_, alpha5_, alpha6_, alpha7_, alpha8_, alpha9_, SF_;
  ...
  //beam skip related params
    ...
    void reconfigureCB(omnirob_robin_amcl::omnirob_robin_AMCLConfig &config, uint32_t level);
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("odom_alpha7", alpha7_, 0.2);
  private_nh_.param("odom_alpha8", alpha8_, 0.2);
  private_nh_.param("odom_alpha9", alpha9_, 0.2);
  private_nh_.param("odom_SF", SF_, 1.0);
  ...
  else if(tmp_model_type == "omni-carapia")
    odom_model_type_ = ODOM_MODEL_OMNI_CARAPIA;
  else
  {
  ...
  dsrv_ = new dynamic_reconfigure::Server<omnirob_robin_amcl::omnirob_robin_AMCLConfig>(ros::NodeHandle("~"));
  dynamic_reconfigure::Server<omnirob_robin_amcl::omnirob_robin_AMCLConfig>::CallbackType cb = boost::bind(&AmclNode::reconfigureCB, this, _1, _2);
...
void AmclNode::reconfigureCB(omnirob_robin_amcl::omnirob_robin_AMCLConfig &config, uint32_t level)
{
  ...
  alpha8_ = config.odom_alpha8;
  alpha9_ = config.odom_alpha9;
  SF_ = config.odom_SF;
  ...
  if(config.odom_model_type == "diff")
    ...
    odom_model_type_ = ODOM_MODEL_OMNI_CORRECTED;
  else if(config.odom_model_type == "omni-carapia")
    odom_model_type_ = ODOM_MODEL_OMNI_CARAPIA;
  ...
  odom_ = new AMCLOdom();
  ROS_ASSERT(odom_);
  odom_->SetModel( odom_model_type_, alpha1_, alpha2_, alpha3_, alpha4_, alpha5_, alpha6_, alpha7_, alpha8_, alpha9_, SF_ );
  // Laser
  delete laser_;
