...
//Function introduced to model the omnidirectional motion

  case ODOM_MODEL_OMNI_CARAPIA:
  {
    // Implement sample_motion_odometry
    double delta_xR, delta_yR, delta_thetaR;
    double delta_xR_hat, delta_yR_hat, delta_thetaR_hat;
    double delta_x, delta_y, delta_theta;
    double theta_p, theta_p_hat;

    // Avoid computing a bearing from two poses that are extremely near each
    // other (happens on in-place rotation).
    
    delta_x=ndata->delta.v[0];
    delta_y=ndata->delta.v[1];
    delta_theta=ndata->delta.v[2];

    theta_p=old_pose.v[2]+delta_theta/2;

    delta_xR=delta_x*cos(theta_p)+delta_y*sin(theta_p);
    delta_yR=delta_y*cos(theta_p)-delta_x*sin(theta_p);
    delta_thetaR=delta_theta;
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      delta_xR_hat=delta_xR+pf_ran_gaussian(this->SF*sqrt(this->alpha1*this->alpha1 +  this->alpha2*this->alpha2*delta_yR*delta_yR + this->alpha3*this->alpha3*delta_thetaR*delta_thetaR));

      delta_yR_hat=delta_yR+pf_ran_gaussian(this->SF*sqrt(this->alpha4*this->alpha4*delta_xR*delta_xR +  this->alpha5*this->alpha5*delta_yR*delta_yR + this->alpha6*this->alpha6*delta_thetaR*delta_thetaR));

      delta_thetaR_hat=delta_thetaR+pf_ran_gaussian(this->SF*sqrt(this->alpha7*this->alpha7+  this->alpha8*this->alpha8*delta_yR*delta_yR + this->alpha9*this->alpha9*delta_thetaR*delta_thetaR));

      theta_p_hat=sample->pose.v[2]+delta_thetaR_hat/2;

      // Apply sampled update to particle pose

      sample->pose.v[0] += delta_xR_hat * cos(theta_p_hat)-delta_yR_hat*sin(theta_p_hat);
      sample->pose.v[1] += delta_yR_hat * cos(theta_p_hat)+delta_xR_hat*sin(theta_p_hat);
      sample->pose.v[2] += delta_thetaR_hat;
